...
At 10:19:09 p.m. on 28 July 2021 (UTC−6; 04:19:09 UTC the next morning), Extra Found Files began taking recovery-output timestamps. At 10:46:08 p.m. the window closed: **177,488** recovered paths, **295.76 GB** of logical data, **105,654** distinct SHA-256 values. Wall clock: 26 minutes 59 seconds. Split at pauses of 15 idle seconds and the run is eight batches. Drop 427 idle seconds and the active span is **1,193 seconds** — 19 minutes 53 seconds. [July 28 burst](docs/JULY_28_BURST.md).

Hunter Biden’s MacBook is attributed a **256 GB** internal SSD. The burst already reports **~40 GB more** logical data than that drive can hold as unique payload. Across those 1,193 active seconds the implied logical rate is **~248 MB/s**. Gigabit Ethernet’s theoretical line rate is **125 MB/s**. A consumer hard disk writing one sequential stream is slower than either number. 248 MB/s is logical recovery output, not a measured disk or network transfer: duplicate paths, overlapping recoveries, and many files sharing a one-second timestamp inflate the sum. Treated as unique payload, the burst does not fit the attributed SSD. Treated as a network copy, it outruns gigabit Ethernet. One batch stamped **124,525** files in 8 minutes 34 seconds. Another assigned **14.2 GB** to a single second.

**0728** is that recovery-export tree: R-Studio Known File Types folders, synthetic seven-digit names, no volume header, no original paths. The scanned medium, the R-Studio project, and the operator log are not in the tree. Several hundred gigabytes therefore circulate without a filesystem that would identify their source. Garrett Ziegler sent this author a MEGA link in August 2021 that he said **Conan James Hayes** had given him. The author is the **first person known to download** from that link. Hayes later said on a recorded call that he carved the Extra Files from deleted unallocated space. That account is **technically false** as the origin of the unmatched remainder. [Hayes carving claim](docs/HAYES_CARVING_CLAIM.md). On the same call Hayes said he had helped the FBI on Backpage and worked for Homeland, coming in and out, not associated or affiliated. Those words are an admitted government connection. They are not a registered FBI confidential-informant file. [Informant theory](docs/INFORMANT_THEORY.md).

This GitHub repository is an encyclopedia of the corpus: tree identity, category inventories, SHA-256 overlap with other indexed sources, timestamps, and a sourced Hayes custody-and-method history. It does **not** publish the Extra Found Files bytes.
