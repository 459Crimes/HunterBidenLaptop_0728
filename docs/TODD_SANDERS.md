# Todd Sanders

> **Hatnote.** Working hypothesis for **how a bootable laptop copy reached Hayes**. Not the MEGA hop for Extra Found Files. Chat-based unless otherwise labeled.

**Todd Sanders** sits at the intersection of the **America Project** election-audit network and the **APFS bootable** laptop copies that Hayes later used.

## Why he is on the 0728 page

0728 itself arrived via [Ziegler](GARRETT_ZIEGLER.md). The **disk Hayes scanned** (if Extra Files came from a laptop-lineage copy at all) is a different question. Parent-project research treats:

```text
Mac Isaac  →  Mike Trimarco  →  Todd Sanders (bootable Desktop copy)
                                      │
                                      ▼
                                 Conan Hayes  →  MPOLO / APFS*
```

as the **best current network** for APFS-lineage media. Hayes then appears as alleged producer of Extra Found Files. Sanders is therefore **upstream of Hayes’s working laptop**, not the August 2021 MEGA courier.

## What Sanders said (July 2026)

Informal, retrospective, not a sworn custody log:

- “ours was from **[Mike] Trimarco**”;
- bootable copy with contents “dropped on the **Desktop**”;
- Mac Isaac talked recipients through a **folder copy**, not a clone;
- Sanders later made a **forensic image and clone**;
- he cannot distribute the pristine Mac Isaac copy under agreement with Mac Isaac’s attorney;
- he **can** share derived hashes / counts (and later did: JPMI metadata ZIP to this author, **July 2026**).

The chat does **not** attach a date, source hash, or sentence that Conan received Todd’s Trimarco copy. That last hop remains **attribution**.

## Public network

American Oversight records place Sanders in America Project “Fund the Audit” leadership and in communications with Doug Logan / Cyber Ninjas. The same records place **Hayes** and **Byrne** in that orbit. Network ≠ bill of lading.

## Name note

Chat: “[Mike] Trimarco.” Journalism: “Michael” / “Mike” Trimarco ([Raw Story, 13 Apr 2022](https://www.rawstory.com/rudy-giuliani-associate/)).

## See also

- [Copy lineages](COPY_LINEAGES.md)
- [Conan Hayes](CONAN_HAYES.md)
- [Patrick Byrne](PATRICK_BYRNE.md)
- [iPhone backup password](IPHONE_BACKUP_PASSWORD.md) (Hayes’s fuller copy vs MPOLO)
